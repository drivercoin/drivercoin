...
*Setup instructions for a gitian build of Drivercoin using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Drivercoin
...
- [Building Drivercoin](#building-drivercoin)
...
- [LXC](https://linuxcontainers.org/), see also [Gitian host docker container](https://github.com/gdm85/tenku/tree/master/docker/gitian-drivercoin-host/README.md).
...
Clone the git repositories for drivercoin and gitian.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/drivercoin/drivercoin
```
...
in the drivercoin repository under 'Fetch and build inputs' to install sources which require
...
To build Drivercoin (for Linux, OSX and Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the drivercoin repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/drivercoin/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/drivercoin/drivercoin
...
For example:
```bash
URL=https://github.com/laanwj/drivercoin.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit drivercoin=${COMMIT} --url drivercoin=${URL} ../drivercoin/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit drivercoin=${COMMIT} --url drivercoin=${URL} ../drivercoin/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit drivercoin=${COMMIT} --url drivercoin=${URL} ../drivercoin/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the drivercoin git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=precise on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../drivercoin/contrib/gitian-descriptors/*|sort|uniq )
...
DVRPATH=/some/root/path/drivercoin.git
SIGPATH=/some/root/path/drivercoin-detached-sigs.git

./bin/gbuild --url drivercoin=${DVRPATH},signature=${SIGPATH} ../drivercoin/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/drivercoin-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/drivercoin-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/drivercoin-osx-build.assert
```
...
[drivercoin/gitian.sigs](https://github.com/drivercoin/gitian.sigs/) repository, or if that's not possible create a pull
