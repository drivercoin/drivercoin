    ...
    platformStyle(platformStyle)
{
    ...
    ui->signatureOut_SM->setFont(GUIUtil::drivercoinAddressFont());
    ui->signatureIn_VM->setFont(GUIUtil::drivercoinAddressFont());
}
...
void SignVerifyMessageDialog::on_signMessageButton_SM_clicked()
{
    ...
    CDrivercoinAddress addr(ui->addressIn_SM->text().toStdString());
    if (!addr.IsValid())
    {
...
{
    CDrivercoinAddress addr(ui->addressIn_VM->text().toStdString());
    if (!addr.IsValid())
    {
    ...
    if (!(CDrivercoinAddress(pubkey.GetID()) == addr))
    {
        ui->statusLabel_VM->setStyleSheet("QLabel { color: red; }");
